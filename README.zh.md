...
从课标抽出 1,334 条候选能力锚点，**只有 143 条**通过了「判定客观、无需教师复核」的门槛并随包分发：
...
这 143 条集中在**语文识字/写字/背诵**与**英语词汇**——因为这些能力的对错是客观的：字写对没有、篇背下来没有。

**数学、物理、化学等学科的锚点查不到。** 它们的判定依赖教学判断（如「能运用数形结合思想」），还没有教师复核过，因此没有纳入可用集合。`k12_substrate_info` 会如实报告这条边界，`k12_find_capability` 查数学会诚实地返回 0 条——**查不到不等于课标里没有**。

同理，本插件**不能回答「先学什么后学什么」**。可用锚点之间只有 2 条依赖边，且都来自实测的集合包含关系（基本字表 ⊂ 常用字表一，实测 95%；英语二级词汇 ⊂ 三级词汇，实测 100%），那不是学习路径。
...
你：帮我看看「灞」这个字，小学要求会写吗？
  ...
  「灞」不在课标附录的任何清单里（已查 8 张表：常用字表 3500、基本字表 299、
   背诵篇目 135、英语二级/三级词汇等）。
...
**3. 只能引用可用锚点。**
断言指向一条没人复核过的锚点，等于用没验过的尺子量孩子。传了不在这 143 条里的 ID 会直接报错。
