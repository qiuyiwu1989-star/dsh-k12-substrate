 ...
 *
 * **这段 2026-08-20 改过 —— 原文说「可用锚点之间只有 2 条依赖边」，已经是假的。**
 * 底座那边完成了边的语义重标（specs/001），快照里现在有一千多条推理图边，
 * 每条带 type（子动作 / 手段可绕 / 概念前提）和不具备时的具体失败表现。
 *
 * 但**这个工具的「下一步」仍然只用清单**，两个来源：
 *   - 集合包含边 —— 先把小表吃完再上大表
 *   - 表内序号 —— 课标附录的表本身是有序的
 *
 * 为什么不用那一千多条边来排学习路径：它们全是 `llm-proposed`，**没有任何人看过**。
 * 拿没人复核的边给一个具体孩子排路径，就是把没验过的东西当验过的用 ——
 * 这和当初不肯用未复核锚点是同一条线。边的语义已经透给 k12_find_capability，
 * 模型解释「为什么先学这个」时用得上；但**由它自动排路径，等有人复核之后再说**。
 */
